      ...
      <RackFrame className={`digitone-module instrument-module ${instrumentMutes.digitone ? 'module-muted' : ''}`}>
        <div className="unit-heading instrument-heading">
          <div className="module-ident">
            ...
            <div><span className="unit-type">INSTRUMENT MODULE 01</span><h1>DIGITONE</h1></div>
          </div>
        ...
        <SequenceToolbar label="Digitone" view={digitoneView} page={digitonePage} onView={setDigitoneView} onPage={(nextPage) => { setDigitonePage(nextPage); setSelectedStep((current) => ({ ...current, index: nextPage * pageSize })) }} />
      ...
      <RackFrame className={`digitakt-module instrument-module ${instrumentMutes.digitakt ? 'module-muted' : ''}`}>
        <div className="unit-heading instrument-heading">
          <div className="module-ident">
            ...
            <div><span className="unit-type">INSTRUMENT MODULE 02</span><h1>DIGITAKT</h1></div>
          </div>
        ...
        <div className="module-body">
        <SequenceToolbar label="Digitakt" view={digitaktView} page={digitaktPage} onView={setDigitaktView} onPage={(nextPage) => { setDigitaktPage(nextPage); setSelectedDrumStep((current) => ({ ...current, index: nextPage * pageSize })) }} />
        <div className="drum-lanes">
          {digitaktTracks.map((track) => <DigitaktLane key={track.id} track={track} view={digitaktView} page={digitaktPage} selected={selectedDrumStep.trackId === track.id ? selectedDrumStep.index : null} currentStep={currentSteps[track.id] ?? null} onSelect={(index) => { setSelectedDrumStep({ trackId: track.id, index }); setDigitaktPage(Math.floor(index / pageSize)) }} onChange={(change) => updateDigitaktTrack(track.id, change)} />)}
        </div>
...
function SeedLab({ settings, onSettings, onSeed, lastSeed, seedCount, busy }: { settings: SeedSettings; onSettings: (settings: SeedSettings) => void; onSeed: () => void; lastSeed: string; seedCount: number; busy: boolean }): React.JSX.Element {
  ...
  return <RackFrame className="seed-module">
    <div className="unit-heading">
      <div><span className="unit-type">PHRASE ENGINE V2</span><h1>SEQUENCE GENERATOR</h1></div>
      <div className="seed-count"><span>PHRASES</span><strong>{String(seedCount).padStart(2, '0')}</strong></div>
    </div>
    <div className="seed-controls">
      ...
      <button className={`seed-action ${busy ? 'working' : ''}`} aria-busy={busy} onClick={onSeed}><span>✦</span><strong>GENERATE</strong></button>
    </div>
...
function LfoRack({ lfos, levels, onChange }: { lfos: LfoConfig[]; levels: Record<LfoId, number>; onChange: (id: LfoId, change: (lfo: LfoConfig) => LfoConfig) => void }): React.JSX.Element {
  return <RackFrame className="lfo-module">
    <div className="unit-heading">
      <div><span className="unit-type">MODULATION SOURCE MODULE 00</span><h1>GLOBAL LFOs</h1></div>
...
function ModuleSetup({ target, outputs, selected, tracks, onSelect, onChannel }: { target: RackTarget; outputs: string[]; selected: number | null; tracks: Array<{ id: TrackId; label: string; channel: number }>; onSelect: (target: RackTarget, port: number | null) => Promise<void>; onChannel: (trackId: TrackId, channel: number) => void }): React.JSX.Element {
  return <div className="module-setup">
    <div className="module-routing">
      ...
      <details className="channel-config">
...
function SequenceToolbar({ label, view, page, onView, onPage }: { label: string; view: SequenceView; page: number; onView: (view: SequenceView) => void; onPage: (page: number) => void }): React.JSX.Element {
  return <section className="sequence-toolbar" aria-label={`${label} sequence view`}>
    <span className="section-label">4 BARS · 64 STEPS</span>
...
function DigitoneLane({ track, view, page, selected, currentStep, lfoLevels, onSelect, onChange, onMacro, onMacroRoute, onMacroDepth }: { track: DigitoneTrack; view: SequenceView; page: number; selected: number | null; currentStep: number | null; lfoLevels: Record<LfoId, number>; onSelect: (index: number) => void; onChange: (change: (track: DigitoneTrack) => DigitoneTrack) => void; onMacro: (macro: 'tone' | 'space', value: number) => void; onMacroRoute: (macro: 'tone' | 'space', source: LfoId | 'manual') => void; onMacroDepth: (macro: 'tone' | 'space', depth: number) => void }): React.JSX.Element {
...
function DigitaktLane({ track, view, page, selected, currentStep, onSelect, onChange }: { track: DigitaktTrack; view: SequenceView; page: number; selected: number | null; currentStep: number | null; onSelect: (index: number) => void; onChange: (change: (track: DigitaktTrack) => DigitaktTrack) => void }): React.JSX.Element {
